    ...
    for isoi in isoform_info:
      ...
      if len(exons) == len(starts) == len(ends):
        for index, exon in enumerate(exons):
          all_exons.append({"id": exon, "start": int(starts[index]), "end": int(ends[index])})

    if len(all_exons) > 0:
      merged_ranges = GFFHandler.merge_exon_ranges(all_exons)
    else:
      merged_ranges = []

    gene = {'name': gene_info['ID'],
