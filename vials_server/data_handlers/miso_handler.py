...
class MisoHandler:
  ...
  def get_samples_and_measures(self, gene_id, gene_meta):
    ...
    for sample in all_samples:
      ...
      samples_info[sample_id] = {'id': sample_id, 'type': 'miso', 'meta': {}}

      isoforms = sample['isoforms'].split(',')
      isoforms = map(lambda iso: iso.strip("'"), isoforms)
      isoforms = map(lambda iso: isoform_long_to_short_names[iso] if (iso in isoform_long_to_short_names) else iso,
                     isoforms)
      ...
      if len(isoforms) == len(measures):
        for index, isoform in enumerate(isoforms):
          iso_measures.append({'id': isoform, 'sample': sample_id, 'weight': measures[index]})
      ...
      if sample_id in sample_meta:
        ...
        if bam_file and not bam_file == '':
          ...
          if cache_hit:
            ...
            jxn, wiggles = self.get_wiggles_and_jxns(os.path.join(self.project_info['info']['bam_root_dir'], bam_file),
                                                     gene_meta,
                                                     2000)
