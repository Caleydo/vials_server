...
class BodyMapHandler:
  def __init__(self, data_root):
    ...
    self.summary_file_in_project = lambda project, sample: os.path.join(project, "summary",
                                                                        "%s.miso_summary.json" % sample)
  ...
  def generate_meta_info(self, gene_name, project, datagroup):
    ...
    for x in gene_obj.parts:
      ...
      exon_map[x.label] = ex_info

    # merge exons
    merged_ranges = self.merge_exon_ranges(exon_map)
    # create a mapping between merged exon version and new name
    exon_exon_map = {}
    for newExon in merged_ranges:
      for oldExon in newExon["names"]:
        exon_exon_map[oldExon] = newExon

    # add (reference) isoforms
    isoforms = {}
    self.isoformIDMap = {}
    for x in gene_obj.isoforms:
      related_exons = x.desc  # map(lambda exonName: exon_exon_map[exonName], x.desc)
    ...
    # self.exon_exon_map = exon_exon_map
    # self.exon_map = exon_map

    # todo: not nice but works :)
    datagroup["isoform_unit"] = "estimated Psi (Percent Spliced In)"

    return chrom_id, strand, tx_end, tx_start, exon_map, isoforms, merged_ranges

  def read_data(self, gene_name, add_reads, all_jxns_ends, all_jxns_starts,
                all_sapmple_infos, datagroup, isoform_measured, jxns, sample_reads, project,
                tx_start, tx_end, chrom_id, resampled=2000):
    ...
    for sample, sample_info in datagroup['samples'].iteritems():
      ...
      with open(summary_file_name, 'rb') as miso_file:
        ...
        for isodesc in miso_info["isoforms"]:
          ...
          isoform_measured.append({"id": isodesc, "weight": mean, "sample": sample})
      ...
      sample_jxns_file_name = os.path.join(project['dir'], sample, gene_name,
      ...
      if add_reads:
        sample_reads_file_name = os.path.join(project['dir'], sample, gene_name,
                                              "wiggles.json")  # TODO: create a head function for that
        with open(sample_reads_file_name) as reads_file:
          all_samples = json.load(reads_file)
          if resampled:
            all_samples = resample(all_samples, resampled).tolist()
